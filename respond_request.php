...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['user_id'])) {
    ...
    // Update the request status (execute once)
    ...
    if ($ok) {
        // Get who sent the request (requester)
        ...
        if ($action == 'accepted') {
            ...
            $msg = 'Your study request was accepted.';
            $notif->bind_param("iisis", $requester_id, $user_id, $type, $request_id, $msg);
            $notif->execute();
        } elseif ($action == 'declined') {
            // Notify the requester that their request was declined
            $notif = $conn->prepare("INSERT INTO Notifications (UserID, ActorID, Type, ItemID, Message) VALUES (?, ?, ?, ?, ?)");
            $type = 'request_declined';
            $msg = 'Your study request was declined.';
            $notif->bind_param("iisis", $requester_id, $user_id, $type, $request_id, $msg);
            $notif->execute();
        }
    }
